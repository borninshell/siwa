 ...
 */
export function serializeMessage(message: SIWAMessage): string {
  const lines: string[] = [];
 ...
 */
export function parseMessage(text: string): SIWAMessage {
 ...
 */
export function validateTiming(message: SIWAMessage): { valid: boolean; error?: string } {
  const now = new Date();
