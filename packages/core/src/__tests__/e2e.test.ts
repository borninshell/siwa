...
describe('SIWA End-to-End Flow', () => {
  ...
  const nonceStore = new Map<string, { address: string; message: string; expiresAt: Date }>();
  ...
  describe('Attack Scenarios', () => {
    it('should reject signature from different key', async () => {
      ...
      const result = await verify(
        messageText,
        bs58.encode(attackerSig),
        bs58.encode(attackerKeypair.publicKey),
        { skipTimeCheck: true }
      );
