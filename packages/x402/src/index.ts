...
export function createX402Middleware(options: X402Options) {
  ...
  return async function x402Middleware(
    req: Request & { agentAddress?: string; paymentProof?: PaymentProof },
    res: Response,
    next: NextFunction
  ) {
...
export async function payAndRetry(
  ...
  const requirements: PaymentRequirements = JSON.parse(reqHeader);

  // Create and send payment
  const { transaction, amount } = await createPaymentTransaction(
    connection,
    keypair,
    requirements
  );
 ...
 */
export function getPaymentSession(agentAddress: string): PaymentSession | undefined {
